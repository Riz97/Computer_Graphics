...
Object * find_nearest_object(const Scene &scene, const Ray &ray, Intersection &closest_hit) {
    ...
    for (int i = 0; i < scene.objects.size(); i++) {
		...
		//if a ray hits any of the objects in the scene 
        if (scene.objects[i]->intersect(ray, hit)) {
			//save the object values but if it is not the nearest , it will ignore it
...
bool is_light_visible(const Scene &scene, const Ray &ray, const Light &light) {
	// DONE : Determine if the light is visible here (Assignment 2, shadow ray)
            ...
            Vector3d C(0, 0, 0);
				...
				for (int l = 0; l < ray_num; l++) {
                    ...
                    if (scene.camera.is_perspective) {
						...
						//The origin of the ray is where the camera is positioned                    
